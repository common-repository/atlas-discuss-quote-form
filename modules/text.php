...
function adf_custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
    ...
    switch( $type ) {
        ...
        // date
        case 'date':
            echo '<input type="text" class="datepicker" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . $meta . '"  />
                    <br />' . $desc;
        break;
        // slider
        case 'slider':
        ...
        // image
        case 'image':
            $image = ADF_PLUGIN_DIR . '/images/image.png';    
        ...
        case 'repeatable':
            ...
            foreach( $meta as $row ) {
                ...
                foreach ( $repeatable_fields as $repeatable_field ) {
                    ...
                    echo '<td><label>' . $repeatable_field['label']  . '</label>';
                    echo adf_custom_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
                    echo '</td>';
                } // end each field
                echo '</td><td><a class="meta_box_repeatable_remove repeatable-remove button button-primary button-large" href="#">Remove</a></td></tr>';
                $i++;
            } // end each row
            echo '</tbody>';
            echo '
 ...
 */
if (! function_exists('adf_meta_box_find_field_type')) {
    function adf_meta_box_find_field_type( $needle, $haystack ) {
        foreach ( $haystack as $h )
            if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
                return adf_meta_box_find_field_type( $needle, $h['repeatable_fields'] );
 ...
 * @return  bool                whether or not the type is in the provided array
 */
if (! function_exists('adf_meta_box_find_repeatable')) {
    function adf_meta_box_find_repeatable( $needle = 'repeatable', $haystack ) {
        foreach ( $haystack as $h )
 ...
 */
if (! function_exists('adf_meta_box_santitize_boolean')) {
    function adf_meta_box_santitize_boolean( $string ) {
 ...
 */
if (! function_exists('adf_meta_box_sanitize')) {
    function adf_meta_box_sanitize( $string, $function = 'sanitize_text_field' ) {
        switch ( $function ) {
            case 'intval':
...
if (! function_exists('adf_meta_box_array_map_r')) {
    function adf_meta_box_array_map_r( $func, $meta, $sanitizer ) {
        ...
        foreach( $meta as $key => $array ) {
            ...
            foreach( $array as $arrayKey => $arrayValue )
                if ( is_array( $arrayValue ) )
                    $array[$arrayKey] = adf_meta_box_array_map_r( $func, $arrayValue, $newSanitizer[$arrayKey] );
...
class Custom_Adf_Add_Meta_Box {
    ...
    function admin_enqueue_scripts() {
        ...
        if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
            // js
            $deps = array( 'jquery' );
            if ( adf_meta_box_find_field_type( 'date', $this->fields ) )
                $deps[] = 'jquery-ui-datepicker';
            if ( adf_meta_box_find_field_type( 'slider', $this->fields ) )
                $deps[] = 'jquery-ui-slider';
            if ( adf_meta_box_find_field_type( 'color', $this->fields ) )
                $deps[] = 'farbtastic';
            if ( in_array( true, array(
                adf_meta_box_find_field_type( 'chosen', $this->fields ),
                adf_meta_box_find_field_type( 'post_chosen', $this->fields )
            ) ) ) {
                wp_register_script( 'chosen', ADF_PLUGIN_DIR . '/js/chosen.js', array( 'jquery' ) );
                $deps[] = 'chosen';
                wp_enqueue_style( 'chosen', ADF_PLUGIN_DIR . '/css/chosen.css' );
            }
            if ( in_array( true, array( 
                adf_meta_box_find_field_type( 'date', $this->fields ), 
                adf_meta_box_find_field_type( 'slider', $this->fields ),
                adf_meta_box_find_field_type( 'color', $this->fields ),
                adf_meta_box_find_field_type( 'chosen', $this->fields ),
                adf_meta_box_find_field_type( 'post_chosen', $this->fields ),
                adf_meta_box_find_repeatable( 'repeatable', $this->fields ),
                adf_meta_box_find_field_type( 'image', $this->fields ),
                adf_meta_box_find_field_type( 'file', $this->fields )
            ) ) )
                //wp_enqueue_script( 'meta_box', ADF_PLUGIN_DIR . '/js/scripts.js', $deps );
            
            // css
            $deps = array();
            wp_register_style( 'jqueryui', ADF_PLUGIN_DIR . '/css/jqueryui.css' );
            if ( adf_meta_box_find_field_type( 'date', $this->fields ) || adf_meta_box_find_field_type( 'slider', $this->fields ) )
                $deps[] = 'jqueryui';
            if ( adf_meta_box_find_field_type( 'color', $this->fields ) )
                $deps[] = 'farbtastic';
            //wp_enqueue_style( 'meta_box', ADF_PLUGIN_DIR . '/css/meta_box.css', $deps );
        }
    }
     ...
     */
    function admin_head() {
        if ( in_array( get_post_type(), $this->page ) && ( adf_meta_box_find_field_type( 'date', $this->fields ) || adf_meta_box_find_field_type( 'slider', $this->fields ) ) ) {
    ...
    function meta_box_callback() {
        ...
        foreach ( $this->fields as $field) {
            if ( $field['type'] == 'section' ) {
                ...
                if($field['label'] !== " "){
                echo '<tr>
                    <th><label for="' . $field['id'] . '">' . $field['label'] . '</label></th></tr>';
                }    
                echo '<tr><td>';                        
                    $meta = get_post_meta( get_the_ID(), $field['id'], true);
                    echo adf_custom_meta_box_field( $field, $meta );                        
                echo '</td></tr>';                
            }
        } // end foreach
        ...
        foreach ( $this->fields as $field ) {
            ...
            if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
                ...
                if ( isset( $new ) && '' == $new && $old ) {
                    ...
                    if ( is_array( $new ) )
                        $new = adf_meta_box_array_map_r( 'adf_meta_box_sanitize', $new, $sanitizer );
                    else
                        $new = adf_meta_box_sanitize( $new, $sanitizer );
...
$fields = array(
    array( // Repeatable & Sortable Text inputs
        'label' => 'Field1 Title', // <label>
        'id'    => $prefix.'field1', // field id and name
        'type'  => 'text', // type of field
    ),
    array( // Repeatable & Sortable Text inputs
        'label' => ' ', // <label>
        'id'    => $prefix.'build', // field id and name
        'type'  => 'repeatable', // type of field
        'sanitizer' => array( // array of sanitizers with matching kets to next array
            'featured' => 'adf_meta_box_santitize_boolean',
        ...
        'repeatable_fields' => array ( // array of fields to be repeated
            ...
            'minslider' => array( // jQuery UI Slider
                'label' => 'Min Value', // <label>
                'id'    => 'minvalue', // field id and name
                'type'  => 'text'
            ),
            'maxslider' => array(
                'label' => 'Max Value',
    ...
    array( // Repeatable & Sortable Text inputs
        'label' => 'Field2 Title', // <label>
        'id'    => $prefix.'field2', // field id and name
        'type'  => 'text', // type of field
    ),
    array( // Repeatable & Sortable Text inputs
        'label' => ' ', // <label>
        'id'    => $prefix.'theme', // field id and name
        'type'  => 'repeatable', // type of field
        'sanitizer' => array( // array of sanitizers with matching kets to next array
            'featured' => 'adf_meta_box_santitize_boolean',
            'title' => 'sanitize_text_field',
    ...
    array( // Repeatable & Sortable Text inputs
        'label' => 'Field3 Title', // <label>
        'id'    => $prefix.'field3', // field id and name
        'type'  => 'text', // type of field
    ),
    array( // Repeatable & Sortable Text inputs
        'label' => ' ', // <label>
        'id'    => $prefix.'on_go', // field id and name
        'type'  => 'repeatable', // type of field
        'sanitizer' => array( // array of sanitizers with matching kets to next array
            'featured' => 'adf_meta_box_santitize_boolean',
            'title' => 'sanitize_text_field',
 ...
 */
$sample_box = new custom_adf_add_meta_box( 'sample_box', 'Discuss Form Fields', $fields, 'atlas_discuss_form', true );
